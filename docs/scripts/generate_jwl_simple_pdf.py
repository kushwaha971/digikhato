...
SRC = ROOT / "docs" / "jewellery" / "JWL-SIMPLE-USER-GUIDE.md"
OUT = ROOT / "docs" / "jewellery" / "JWL-SIMPLE-USER-GUIDE.pdf"
...
def build_story(md):
    ...
    code = ParagraphStyle("C", parent=body, fontName="Courier", fontSize=8.5, leading=10.5, backColor=colors.HexColor("#F3F4F6"), borderColor=colors.HexColor("#D1D5DB"), borderWidth=0.5, borderPadding=5)
    title = ParagraphStyle("T", parent=styles["Title"], fontName="Helvetica-Bold", fontSize=22, leading=26, alignment=TA_CENTER, textColor=colors.HexColor("#0F172A"))
    ...
    story = [Spacer(1, 20 * mm), Paragraph("DigiKhaato Jewellery", title), Paragraph("Simple User Guide", title), Spacer(1, 4 * mm), Paragraph("Salesman aur Shopkeeper ke liye", sub), Spacer(1, 8 * mm), Paragraph("Date: 07 May 2026", sub), PageBreak()]
...
def main():
    ...
    doc = SimpleDocTemplate(str(OUT), pagesize=A4, leftMargin=18*mm, rightMargin=18*mm, topMargin=15*mm, bottomMargin=14*mm, title="JWL Simple User Guide")
